({
    ...
    handlePDFClick: function (cmp, event, helper) {

        var publicSiteUrl = cmp.get('v.meta.dto.publicSiteURL');
        var procName = event.getSource().getLocalId();
        var fileName = 'Vertic Content';
        if (publicSiteUrl && procName != null) {
            window.open(publicSiteUrl + '/apex/vertic_Content?proc=' + procName + '&fileName=' + fileName + '&renderAs=pdf', '_blank');
        }
        helper.execute(
            cmp,
            {
